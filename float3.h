...
public:
	...
	float3 elementwise(const float3& operand) const
	{
		return float3(
				x * operand.x,
				y * operand.y,
	...
	bool equals(const float3& other) {
		return x == other.x && y == other.y && z == other.z;
	}

	float absf(float d) const {
		if(d < 0)
			d *= -1;
		return d;
	}
	float3 abs() const {
		return float3(absf(x), absf(y), absf(z));
	}

	float3 power(double factor) const {
		return float3(pow(x,factor), pow(y, factor), pow(z, factor));
	}
};
